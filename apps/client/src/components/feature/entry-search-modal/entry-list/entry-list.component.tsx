...
type TEntryitem = Pick<TParsedEntry, 'title' | 'word_count'> & Pick<TRawEntry, 'id' | 'created_at' | 'updated_at'>
    ...
    useEffect(() => {
        ...
        async function fetchUserEntries({ signal }: { signal: AbortSignal }) {
            try {
                ...
                const parsedEntryList = await Promise.all(entryList.map(async (encryptedEntry) => {
                    const ivArray = Base64Parser.from_base64_to_arraybuffer(encryptedEntry.iv)
                    const dataArray = Base64Parser.from_base64_to_arraybuffer(encryptedEntry.data)

                    const decryptedEntry = await cryptographyApi.decrypt({ iv: ivArray, encryptedData: dataArray })

                    const parsedEntry: TParsedEntry = JSON.parse(decryptedEntry)

                    const entryItem: TEntryitem = { updated_at: encryptedEntry.updated_at, created_at: encryptedEntry.created_at, id: encryptedEntry.id, word_count: parsedEntry.word_count, title: parsedEntry.title }
